...
const CheckoutPage: FC = () => {
    ...
    try {
      ...
      await runTransaction(db, async (transaction) => {
        ...
        for (const item of cart) {
          ...
          if (isExpressItem) {
            continue; // Se salta la lectura/escritura de stock físico
          }
          ...
          if (currentStock < item.quantity) {
            throw new Error(`¡Sin stock suficiente para ${item.title || item.name}! Te quedan ${currentStock} un.`);
          }
        ...
        // 2. Fase de Escritura (Descuenta stock físico)
        ...
        transaction.set(newOrderRef, {
          ...
          items: cart.map((item) => ({
            ...
            image: item.image || "",
            isExpress: item.isExpress || item.id.startsWith("express-") || false,
      ...
      setError(error.message || "Error al procesar la transacción. Intenta nuevamente.");
    ...
    return (
      <div className="min-h-screen bg-orange-50 flex items-center justify-center p-4">
        <div className="bg-white p-10 rounded-2xl shadow-xl text-center max-w-md w-full">
          ...
          <h2 className="text-3xl font-bold text-green-600 mb-3">¡Pedido recibido!</h2>
          <p className="text-gray-600 text-lg mb-2">Tu compra fue procesada con éxito.</p>
          <button
            onClick={() => navigate("/")}
            className="w-full bg-orange-500 hover:bg-orange-600 text-white font-bold py-4 rounded-xl text-lg transition-colors cursor-pointer"
          >
            Volver a la tienda
          </button>
        </div>
    ...
    return (
      <div className="min-h-screen bg-orange-50 flex items-center justify-center p-4">
        <div className="bg-white p-10 rounded-2xl shadow-xl text-center max-w-md w-full">
          ...
          <p className="text-gray-500 mb-8">Agrega productos antes de continuar.</p>
          <button
            onClick={() => navigate("/")}
            className="w-full bg-orange-500 hover:bg-orange-600 text-white font-bold py-4 rounded-xl text-lg transition-colors cursor-pointer"
          >
  ...
  return (
    <div className="min-h-screen bg-orange-50 p-6 pb-28">
      <div className="max-w-2xl mx-auto">
        <h1 className="text-3xl font-bold text-center mb-8 text-gray-800">🧾 Tu pedido</h1>
        ...
        <div className="bg-white rounded-2xl shadow-lg p-6 mb-4">
          <form onSubmit={handleConfirmPurchase} className="space-y-6">
              ...
              {cart.map((item) => {
                ...
                return (
                  <div key={item.id} className="flex items-center gap-4 p-4 bg-gray-50 rounded-xl">
                    <div className="w-16 h-16 flex items-center justify-center rounded-lg bg-white border border-gray-100 shrink-0">
                      {item.image && item.image.trim() !== "" ? (
                        ...
                        <span className="text-3xl">{isVirtual ? "⚡" : "📦"}</span>
                      )}
                    </div>
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full bg-green-500 hover:bg-green-600 disabled:bg-green-300 text-white font-bold py-4 rounded-xl text-xl transition-colors mb-3 cursor-pointer"
            >
            ...
            <button
              ...
              disabled={loading}
              className="w-full bg-yellow-500 hover:bg-yellow-600 disabled:bg-yellow-300 text-white font-bold py-4 rounded-xl text-xl transition-colors mb-3 cursor-pointer"
            >
              ⏸️ Suspender venta
            </button>
